observeEvent(input$do_position, { 
  ...
  text=text2bed(input$position)[1,]
  chrom=text[1]
  start=as.numeric(text[2])
  end=as.numeric(text[3])
  glob_values$chromosome=chrom
  
  log_cat(paste0("Now the user:",getIP(),":search the position for ",input$position))
  ...
  ## we need filter some peaks according to the cellline user choosed.
