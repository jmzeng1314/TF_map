...
cellline=read.table('encode_hg19_cellline_list.txt',sep='\t',stringsAsFactors = F)[,1]
cellline=c('ALL',cellline)
...
page_NewAnalysis <- fluidRow(
  box(title = "search a gene ", status = "info",width=12,
      ...
      selectInput(
        inputId='cellline', label = "Choose a cell line",  
        choices =  as.list(cellline,cellline)  
